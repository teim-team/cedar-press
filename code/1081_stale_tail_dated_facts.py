...
def tail_ueis(tail):
    ...
    for fn, ucol in UEI_SOURCES:
        ...
        # lint-ok: class5 - a SCHEMA guard, not an "already done"
        # short-circuit. It skips a source table that does not carry the
        # column at all, which is a permanent fact about that file and not a
        # resume state, so it cannot make a second run report zero where the
        # first reported work. `write_doc` recomputes every figure from the
        # live ledger and the live output, and two consecutive full runs were
        # measured byte-identical - which is the property the 164 defect
        # violates and this does not.
        if ucol not in hdr:
            continue
        for r in rows:
            if r.get("cedar_uid") in tail:
...
# ----------------------------------------------------------------------- doc
# The BEFORE column is a fixed record of a measurement that no longer exists on
# disk, so it cannot be re-derived and is stated with the file it came from.
# Everything else in the report is computed at write time.
BEFORE = {"untouched_over_a_year": 287, "no_usable_date": 373,
          "no_substantive_row": 83, "median_days": 112, "p90_days": 3627,
          "basis": ("`docs/ENTITY_FRESHNESS.md` as generated 2026-09-02 before "
                    "this workstream; that file is kept at "
                    "`docs/ENTITY_FRESHNESS.md"
                    ".bak_2026-09-02_pre_1081_stale_tail_dated_facts`")}
OUT_DOC = os.path.join(ROOT, "docs", "STALE_TAIL_CLOSURE_1081.md")

REASON_MEANING = {
    "NO_IRS_ORGANISATION_FOR_THIS_NAME":
        "`SOURCE_DOES_NOT_PUBLISH`. The IRS Exempt Organizations file holds no "
        "organisation of this name in this state. Most are Native Hawaiian "
        "homestead associations and *hui* that are not 501(c)(3) filers.",
    "NOT_IN_CCD_BIE_UNIVERSE":
        "`SOURCE_DOES_NOT_PUBLISH`. NCES has never carried this school in the "
        "BIE reporting universe in any year 2000-2024.",
    "NO_IDENTIFIABLE_NAME":
        "The name is made only of stopwords and generic words, so no name "
        "check can identify it. This is the *Council* shape.",
}


def reason_meaning(k):
    if k in REASON_MEANING:
        return REASON_MEANING[k]
    if k.startswith("AMBIGUOUS_"):
        return ("more than one NCES BIE school number matches this name "
                "(usually a school that split or consolidated); picking one "
                "would be a guess, so no date is written")
    if k.startswith("NO_EXACT_NAME_IN_"):
        return ("the IRS returns organisations for this name but none whose "
                "distinctive token set is EQUAL to the entity's. Overlap "
                "against the smaller side is not identity, and accepting it "
                "produced four wrong EINs when it was tried")
    if k.startswith("tokens "):
        return ("an EIN Cedar holds on disk resolves at the IRS to a "
                "DIFFERENT organisation, so its filing dates are not this "
                "entity's. The on-disk link is tier B and was not inherited")
    return "recorded on the row"


def _pctl(v, q):
    v = sorted(v)
    return v[min(len(v) - 1, int(round(q * (len(v) - 1))))] if v else 0


def write_doc():
    rows = read_freshness()
    now = measure(rows)
    days = [int(r["days_since_change"]) for r in rows if r["days_since_change"]]
    with open(OUT, encoding="utf-8-sig", newline="") as fh:
        facts = list(csv.DictReader(fh))
    dated = [f for f in facts if f["as_of_date"]]
    cutoff = (date.fromisoformat(TODAY)
              - timedelta(days=STALE_BAR_DAYS)).isoformat()
    ents_dated = {f["cedar_uid"] for f in dated}
    ents_recent = {f["cedar_uid"] for f in dated if f["as_of_date"] >= cutoff}

    L = []
    A = L.append
    A("# Closing the stale-entity tail \u2014 what was acquired, and what is "
      "genuinely dateless")
    A("")
    A("*Generated by `py -3 code/1081_stale_tail_dated_facts.py doc`. Every "
      "date in `data/clean/entity_dated_public_facts.csv` is stated by the "
      "source named on its row, and `as_of_date_basis` names the field that "
      "states it. Cedar's own clock is `checked_date`, which 830's `NEVER` "
      "regex excludes by name and which `verify` refuses to let any "
      "`as_of_date` equal.*")
    A("")
    A("## The three numbers")
    A("")
    A("| measure | before | after |")
    A("|---|---:|---:|")
    for k, lab in (("untouched_over_a_year", "untouched over a year"),
                   ("no_usable_date", "no usable date at all"),
                   ("no_substantive_row", "in no substantive Cedar row")):
        A("| %s | %d | **%d** |" % (lab, BEFORE[k], now[k]))
    A("| *the union of the first two* | %d | **%d** |"
      % (BEFORE["untouched_over_a_year"] + BEFORE["no_usable_date"],
         now["untouched_over_a_year"] + now["no_usable_date"]))
    A("| median days since change | %d | %d |"
      % (BEFORE["median_days"], _pctl(days, 0.5)))
    A("| p90 days | {:,} | {:,} |".format(BEFORE["p90_days"], _pctl(days, 0.9)))
    A("")
    A("Before is " + BEFORE["basis"] + ".")
    A("")
    A("**`untouched over a year` went UP, and that is the instrument working "
      "rather than a regression.** 287 was an undercount: an entity with no "
      "date at all could not be counted as stale, because there was nothing "
      "to compare against. %d entities gained a first real date here and most "
      "of those dates are old, so they moved out of the invisible bucket and "
      "into the visible one. The figure that cannot move in that way is the "
      "union of the two, and it fell by %d. So did p90, by {:,} days \u2014 "
      "the extreme tail is shorter."
      .format(BEFORE["p90_days"] - _pctl(days, 0.9))
      % (len(ents_dated),
         (BEFORE["untouched_over_a_year"] + BEFORE["no_usable_date"])
         - (now["untouched_over_a_year"] + now["no_usable_date"])))
    A("")
    A("## What was acquired, and from where")
    A("")
    A("| route | source | rows | entities dated | within a year |")
    A("|---|---|---:|---:|---:|")
    for rt in sorted({f["route"] for f in facts}):
        rr = [f for f in facts if f["route"] == rt]
        dd = [f for f in rr if f["as_of_date"]]
        A("| `%s` | %s | %d | %d | %d |"
          % (rt, rr[0]["source"].split(" via ")[0][:70], len(rr),
             len({f["cedar_uid"] for f in dd}),
             len({f["cedar_uid"] for f in dd if f["as_of_date"] >= cutoff})))
    A("")
    A("%d entities received a dated public fact in total; %d of those dates "
      "fall inside the 365-day bar." % (len(ents_dated), len(ents_recent)))
    A("")
    A("## Where a date could NOT be found, and why")
    A("")
    A("*Each of these is a row in the table with `match_method = NOT_MATCHED` "
      "and its reason, so the negative is arguable rather than invisible.*")
    A("")
    A("| reason | n | what it means |")
    A("|---|---:|---|")
    why = defaultdict(int)
    for f in facts:
        if f["match_method"] == "NOT_MATCHED":
            why[f["match_note"].split(":")[0].split("(")[0].strip()] += 1
    for k, n in sorted(why.items(), key=lambda x: -x[1]):
        A("| `%s` | %d | %s |" % (k, n, reason_meaning(k)))
    A("")
    A("### The classes that remain genuinely dateless")
    A("")
    A("| class | still stale | still undated |")
    A("|---|---:|---:|")
    st, nd = defaultdict(int), defaultdict(int)
    for r in rows:
        if not r["last_change"]:
            nd[r["entity_class"]] += 1
        elif int(r["days_since_change"]) > 365:
            st[r["entity_class"]] += 1
    for c in sorted(set(st) | set(nd), key=lambda c: -(st[c] + nd[c])):
        A("| %s | %d | %d |" % (c, st[c], nd[c]))
    A("")
    A("## Three findings about the sources")
    A("")
    A("**1. The ANCSA annual-report corpus does not reach the ANVC tail.** "
      "The brief said 358 audited AS 45.55.139 reports at "
      "`data/interim/ancsa_txt_v3/` carry dated financials for the stale "
      "Alaska Native village corporations. Measured: those 358 reports come "
      "from **41** corporations, and exactly **one** of the 95 tail "
      "ANVC / ANCSA-group entities is among them \u2014 Gwitchyaa Zhee, "
      "already dated 2024-12-31. AS 45.55.139 exempts the small village "
      "corporations, which is precisely the population in the tail. Re-mining "
      "the corpus would produce nothing.")
    A("")
    A("**2. The one register that would date all 95 is behind a bot wall.** "
      "`www.commerce.alaska.gov/cbp/...` answers **HTTP 403 with a DataDome "
      "CAPTCHA** \u2014 to `robots.txt`, to the entity search and to the bulk "
      "`CorporationsDownload.CSV` \u2014 measured 2026-09-02. Every Alaska "
      "corporation has a stated registration date, status and biennial-report "
      "date there. State: **NOT_ACQUIRED, host refuses automation**, which is "
      "not `SOURCE_DOES_NOT_PUBLISH`. A human-driven or records-request route "
      "would close 95 entities from one file and is the highest-value "
      "acquisition left in this tail.")
    A("")
    A("**3. ProPublica answers a zero-result search with HTTP 404.** Measured "
      "the same day: `q=alu+like` returns 200 with one organisation, "
      "`q=ahfachkee+school` returns 404, `q=school` returns 200. The 404 is a "
      "fact about the QUERY, not about the host, and the first run of the "
      "search route counted three of them as a host refusal and stopped after "
      "two rows. The cause of those three was ours: `urllib.parse.quote` "
      "defaults to `safe=\"/\"`, so *Baca /Dlo'Ay Azhi Community School* "
      "produced a query string with a live slash in it.")
    A("")
    A("## Why NCES cannot make a BIE school look fresh")
    A("")
    A("CCD collection year Y is school year Y/Y+1 and its snapshot is the "
      "1 October membership count date. The newest year NCES publishes for "
      "fips 59 is **2024**, so the newest possible CCD date is "
      "**2024-10-01** \u2014 outside the 365-day bar by construction. And the "
      "BIE reporting universe is **static**: the same 174 schools reported in "
      "every collection year 2008-2024, so \"the most recent year this school "
      "reported\" is one date shared by all of them. That is the shape 830 "
      "refuses as a build stamp, and this script's own `verify` refuses it "
      "too. There is one narrow exemption, `uniform_source_date = Y`, and it "
      "is available only to dates **outside** the staleness bar, so it can "
      "never make anything look recent. `selftest` proves it is not a "
      "loophole by injecting a declared-uniform date of *today* across 120 "
      "entities and asserting the check still fires.")
    A("")
    A("What CCD does give a BIE school is a first substantive row: an NCES "
      "BIE school number, the operating LEA, enrolment, teacher FTE, "
      "open/closed status and a location. That is what took **in no "
      "substantive Cedar row from 83 to 0**.")
    A("")
    A("## Three identifier traps found on the way")
    A("")
    A("**`ncessch` is not a school id.** It is `leaid` (7 digits) glued to the "
      "school's own 5-digit number, and NCES re-issues the LEA half whenever a "
      "school's reporting agency changes \u2014 Ahfachkee is `590000700041` up "
      "to 2005 and `590009200041` after it, one school, two strings. Keying "
      "identity on it reported **132 of 142** BIE schools as *ambiguous, two "
      "or more NCES ids*, which is a statement about a join key and not about "
      "NCES. Keyed on the last five digits: 116 matched, 26 did not. "
      "`school_id` is not usable either \u2014 the same school is `00041` in "
      "some years and `5900041` in others.")
    A("")
    A("**`GSA_MIGRATION` is not a UEI.** "
      "`fac_tribal_single_audits.auditee_uei` carries that literal string on "
      "rows migrated from the FAC's pre-UEI system, and **40 tail entities "
      "share it**. The ambiguity guard caught it only because it was shared; a "
      "sentinel held by a single entity would have keyed that entity to every "
      "migrated row in the federal tables. A shape test, `^[A-Z0-9]{12}$`, now "
      "runs ahead of the ambiguity test.")
    A("")
    A("**A tier-B UEI is a name guess wearing an exact key.** The UEIs in "
      "`bie_uio_identifier_links.uei` come from a name match against the BIE "
      "roster; that file's own `tier_rationale` says *\"Tier B: the identifier "
      "is not independently confirmed\"*. Treating them as exact links is "
      "START_HERE trap 1 in its purest form. 98 of the 115 tail UEIs are in "
      "that state, and they now earn a date only where the transaction's own "
      "recipient name also matches the entity. That re-check removed 6 "
      "entities and **4,111 prime-contract rows** from the result \u2014 rows "
      "that would have dated an entity from another organisation's contract.")
    A("")
    A("## The self-reference defect, fifth occurrence")
    A("")
    A("830 reads every table in `data/clean`, including this one. So an "
      "entity this script dates leaves the tail \u2014 and on the next run the "
      "slice no longer contains it, its row is replaced away (a route owns and "
      "replaces its own rows), and it returns to `no usable date`. The measure "
      "would oscillate and every run would look like progress. 830 hit this "
      "shape three times on date COLUMNS and 1021 once on row PRESENCE; here "
      "it is the SLICE. `tail_slice` now re-admits every entity already "
      "carried in the output, so the working population is monotone and a "
      "re-run can correct a row but never withdraw one. Proven: two "
      "consecutive full runs produced byte-identical output.")
    A("")
    with open(OUT_DOC, "w", encoding="utf-8") as fh:
        fh.write("\n".join(L) + "\n")
    print("  wrote " + os.path.relpath(OUT_DOC, ROOT))
    return 0

...
def main():
    ...
    _pub = _carry_live_columns(p2, PUBLISHED)
    with open(p2, "w", encoding="utf-8", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=_pub, extrasaction="ignore",
                           restval="")
        w.writeheader()
        w.writerows(awards)
    print(f"  wrote {p2.relative_to(CEDAR)}  ({len(_pub)} columns, "
          f"{len(PUBLISHED)} of them the lean published set)")

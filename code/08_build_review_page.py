...
el('importbutton').onclick=()=>el('importfile').click();el('importfile').onchange=async()=>{try{const value=await el('importfile').files[0].text();if(value.trimStart().startsWith('{'))importRecovery(value);else importDecisions(value);}catch(e){el('receipt').textContent='Import refused: '+e.message;el('exportpanel').open=true;}finally{el('importfile').value='';}};
...
window.needReview={csv,importDecisions,importRecovery,recovery,exportStatus,counts:()=>({events:state.events.length,cases:spec.size}),COLS};refresh();
</script></html>"""

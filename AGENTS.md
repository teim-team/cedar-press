...
0. **Sections 1-8 immediately below** — the operating block shared by every
   ...
   cross-repo picture. ~200 lines, and unlike the rest of this file they are
   edited in place to stay true.
1. **The two sections after that** — the Prime Directive and
...
on their own URLs. Built by Lumecon, available exclusively through Tribal
Business News, which owns payment, renewals and issuance.
...
- Payment, renewals and access-code issuance. Those belong to Tribal Business
  News.
...
- `npm run test:smoke` — Playwright browsers are not installed here. It needs
  `npx playwright install --with-deps chromium` once. Note it **builds as well
  as tests**: `playwright.config.js` starts the server with `npm run build`
  configured with a throwaway demo account, which is why the deployed build is
  the last step of the CI job and not this one.
- `npm run build:site`, `npm run seo:check`, and the pipeline scripts under
  `code/` — the pipeline reaches live sources and rebuilds committed
  deliverables.
...
| **Cedar Grove and Cedar Press** | Havala Hanson (@Havala-Hanson) |
...
| [`teim-engine`](https://github.com/teim-team/teim-engine) | The model engine behind Cedar Impact | This workspace reads its reference data as `reference_dataset` per ADR-044 |
...
## 2026-09-05 — The public dataset specification: an allow list, a map, a retirement rule and twelve guides

The owner's specification (`docs/PUBLIC_DATASET_SPEC_2026-09-05.md`) is in
the repository verbatim, in two parts, and governs the customer files, the
samples, the viewer and the documentation. The rules to keep: the opening
block is `cedar_uid`, `canonical_name`, `entity_class`, `cedar_entity_role`
(Legislation and NAGPRA: the plural block as aligned JSON arrays, never
several ids in one singular cell), the uid is never a record's own id, and
every dataset ends with `research_note`; the export is GENERATED from the
approved field list (`data/cedar/field_map.json`) in the owner's exact
order, never filtered down to it, and a column with no decision fails the
build by name rather than shipping; this pass changes columns and never
rows, identities or eligibility; a `combine` is owed until its sources are
tested for agreement, and a target column that does not exist is absent,
never blank; the codebook lists exactly what ships, under the shipped name;
the viewer reads pipe lists and JSON arrays alike and finds a row through
every declared role. The identifier retirement rule is enforced, not
described: every competing identifier has a disposition, an alias is
verified row by row, an unsettled identifier stops the dataset, a retired
scheme's name in a shipped value stops it, and the regression tests fail if
one returns to a public header. The guides quote figures only from a named

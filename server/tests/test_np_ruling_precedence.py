...
class NonprofitRulingPrecedenceTest(unittest.TestCase):
    ...
    def ruling(self, outcome="UNRESOLVED_ENTITY", target="", status="SETTLED"):
        return dict(
            subject_key="EIN:453221112",
            status=status,
            outcome=outcome,
            source_file="fixture.csv",
            ruling="NATIVE_ORG",
            ruling_date="2026-09-02",
            confidence_tier="A",
            resolved_tribe_id=target,
    ...
    def org(self):
        return dict(
            EIN="453221112",
            org_name="Ahtna Intertribal Resource Commission",
            state="AK",
            tribe_id="ANRC-AHTNAI-00",
            entity_tier="B",
            entity_match_method="containment",
            entity_match_basis="resolver_containment",
            cedar_uid="CE-00076-76",
            disposition="NATIVE_VERIFIED_STRICT",
            evidence="preserved source evidence",
            entity_id="",
        ...
        with (
            patch.object(
                ...
                side_effect=lambda path: (
                    rules if path.name == "cedar_ruling_ledger_consolidated.csv" else rows
                ),
            ),
            ...
            patch.object(p, "rewrite_in_place"),
            patch.object(p, "show", return_value={}),
        ):
            p.do_np_orgs()
    ...
    def candidates(self, rules):
        ...
        ledger = [
            dict(
                identifier_type="UEI",
                identifier="J9NAVAM9B1J3",
                tribe_id="ANRC-AHTNAI-00",
                confidence_tier="B",
                attribution_method="need_v6",
            )
        ]
        with (
            patch.object(h, "NP_ORGS", [self.org()]),
            patch.object(h, "LEDGER", ledger),
            patch.object(
                h,
                "rd",
                side_effect=lambda path: bridge if path.name == "np_ein_uei_bridge.csv" else [],
            ),
        ):
    ...
    def test_current_affirmative_decision_allows_same_target(self):
        ...
        self.assertEqual(
            {r["source"] for r in result["453221112"]}, {"np_orgs", "ein_uei_bridge_to_ledger"}
        )
    ...
    def test_propagation_hold_precedes_stale_hub_and_name_fallback(self):
        ...
        with patch.object(
            self.hub, "deterministic_name_match", side_effect=AssertionError("fallback ran")
        ):
            result = self.hub.apply_nonprofit_link(
                ...
                row["EIN"],
                namecol="org_name",
                hub={"453221112": {"entity_id": "ANRC-AHTNAI-00"}},
                excl={},
                ruling_gate=self.gate,
                decisions=decisions,
                org_identity=True,
            )
    ...
    def test_affirmative_target_cannot_be_replaced_by_name_fallback(self):
        ...
        with patch.object(
            ...
            return_value=("ANRC-AHTNAI-00", "Ahtna", "containment"),
        ):
            result = self.hub.apply_nonprofit_link(
                row,
                "cedar_",
    ...
    def test_hold_preserves_opposite_role_and_issued_ids(self):
        row = dict(
            cedar_uid="CE-OTHER",
            entity_id="SOURCE-OBJECT",
            object_id="FILING-1",
            flow_id="FLOW-1",
            cedar_recipient_spine_entity_id="RECIPIENT",
            cedar_filer_spine_entity_id="WRONG-FILER",
            EIN="453221112",
        )
        decisions = self.gate.build_decisions([self.ruling()])
        self.hub.apply_nonprofit_link(
            row,
            "cedar_filer_",
            row["EIN"],
            hub={},
    ...
    def test_blank_ein_never_looks_up_empty_key(self):
        self.assertEqual(
            self.gate.nonprofit_identity_hold("", {"EIN:": {"action": "RULING_CONFLICT"}}), ""
        )

    def test_blank_ein_keeps_existing_name_fallback(self):
        row = {"org_name": "Independent organization"}
        with patch.object(
            self.hub, "deterministic_name_match", return_value=("", "", "no_spine_match")
        ) as matcher:
            self.hub.apply_nonprofit_link(
                row,
                "cedar_",
    ...
    def test_ein_formats_and_malformed_raw_propagation(self):
        decisions = self.gate.build_decisions([self.ruling()])
        self.assertEqual(
            self.gate.nonprofit_identity_hold("45-3221112", decisions),
        ...
        for raw in ["EIN453221112", "45322111", "45/3221112"]:
            ...
            with patch.object(
                self.hub,
                "deterministic_name_match",
                side_effect=AssertionError("malformed fallback"),
            ):
                result = self.hub.apply_nonprofit_link(
                    {},
                    "cedar_",

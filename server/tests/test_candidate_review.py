...
class CandidateReviewTest(unittest.TestCase):
    def test_accuracy_hold_overrides_stale_publishable_flag(self):
        self.assertEqual(policy.is_publication_eligible({"publish_hold": "Y", "publishable": "Y"}),
    ...
    def test_false_treaty_inclusion_is_held_without_erasing_issued_id(self):
        ...
        self.assertTrue(policy.is_publication_eligible({"bill_id": "99-s-1396", "publishable": "Y"})[0])

    def test_competition_uses_existing_dictionary_and_fails_closed(self):
        rows = [{"extent_competed": "A", "extent_competed_normalized": "FULL AND OPEN COMPETITION"},
                {"extent_competed": "NAN"}]
        header = ["extent_competed", "extent_competed_normalized"]
        policy.recompute_derived("contractors", header, rows)
        self.assertEqual([r["competition_type"] for r in rows], ["FULL AND OPEN COMPETITION", "NOT_REPORTED"])
        self.assertEqual(rows[0]["extent_competed"], "A")
        for row in [{"extent_competed": "unrecognized"},
    ...
    def test_subaward_scientific_homonym_does_not_waive_identity_gate(self):
        ...
        for column, text in [("description", value + " cedar_neid"),
    ...
    def test_subaward_roles_do_not_assert_ownership(self):
        ...
        self.assertEqual([r["role"] for r in entry["entity_roles"]],
    ...
    def test_samples_and_customer_writer_share_row_policy(self):
        ...
        with patch.object(customer, "translate_neid_values"), patch.object(customer, "apply_official_names"), patch.object(customer, "enforce_denials", return_value=0):
            row, reason = customer.publication_row({"title": "safe", "publishable": "N"}, ["title", "publishable"])
            self.assertIsNone(row)
            self.assertEqual(reason, "publishable")
            row, reason = customer.publication_row({"title": "safe", "publishable": "Y", "email": "private"}, ["title", "publishable"])
    ...
    def test_refused_schema_keeps_filtered_preview_but_no_release(self):
        with tempfile.TemporaryDirectory() as directory, ExitStack() as stack:
            ...
            (metadata / "collections.manifest.json").write_text(json.dumps({"collections": [{"id": "legislation", "descriptor": {"tracks": "Bills", "sources": "Fixture"}}]}))
            contracts = base / "contracts.json"
            contracts.write_text(json.dumps({"contracts": [{"collection": "legislation", "tables": [{"table": "native_bills.csv", "primary_key": ["bill_id"]}]}]}))
            queue = base / "queue.json"
            queue.write_text(json.dumps({"collections": [{"collection_id": "fixture-" + str(i), "title": "Fixture", "flagship": "native_bills.csv"} for i in range(12)]}))
            ...
            stack.enter_context(patch.object(policy, "field_map", return_value={"legislation": {"fields": [{"column": x, "decision": "keep"} for x in ("title", "source_url")]}}))
            stack.enter_context(patch.object(policy, "recompute_derived"))
            stack.enter_context(patch.object(policy, "apply_field_map", side_effect=policy.FieldMapRefusal("legislation", [], "fixture refusal")))
            ...
            for result in rows:
                self.assertEqual((result["rows"], result["row_policy_eligible"], result["withheld"]), (3, 2, 1))

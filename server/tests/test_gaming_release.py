...
(``lumecon_data/gaming``, Lumecon-data PR #9 + Gaming hardening) in the
...
class GamingConsumerBoundaryTest(_ServerCase):
    ...
    def test_lumecon_partitioned_payments_parts_are_verified_or_refused_never_assembled(self):
        """Lumecon's bounded-parts shape: each part is an ordinary component and
        ``partitioned_components`` names them with their counts and record
        hashes. Cedar serves no part and no logical partitioned component, and
        a declaration that disagrees with its parts (missing part, changed hash,
        wrong count) refuses the whole pinned release."""
        header = ["payment_id", "amount_usd"]

        def component(sha, count):
            return {
                "rights": {"publication_class": "public", "redistribution": True},
                "download_permitted": True,
                "fields": [{"name": name} for name in header],
        ...
        parts = {
            "gaming_payments__part_a": component("a" * 64, 2),
            "gaming_payments__part_b": component("b" * 64, 3),
        }
        ...
        self.assertEqual(
            ...
            {"gaming_payments", "gaming_payments__part_a", "gaming_payments__part_b"},
        )
        tables = {
            f"gaming/{name}": {"collection": "gaming", "order": header, "fields": []}
            for name in ("gaming_payments", *parts)
        }
        with patch.object(repository, "_field_map_tables", return_value=tables):
            for name in ("gaming_payments", *parts):
                with self.assertRaisesRegex(repository.FullReleaseUnavailable, "partitioned"):
                    repository.grove_component_contract(manifest, "gaming", name)
        incomplete = declared()
    ...
    def setUpClass(cls):
        ...
        if cls.lumecon is None:
            _skip_or_fail(
                ...
                "(Lumecon-data claude/gaming-release-hardening)"
            )

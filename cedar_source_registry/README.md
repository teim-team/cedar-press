...
|---|---|---|
| `sources.jsonl` | 160 | source program (the master registry; 148 wave-5 + TBD-166..177 from the 2026-08 expansion rounds) |
...
| `negative_findings.jsonl` | 210 | Phase 3 formal negatives: tribes checked with no public registry found, with recheck dates |
| `cross_reference.jsonl` | 40 | secondary directory with `do_not_infer` guardrails |
| `verification_log.jsonl` | 140 | append-only fact-check log: 31 wave-5 lines (18 verified, 5 researched-and-excluded, 4 URL corrections, 2 edits reverted) + 97 wave-5.1 search-only re-checks (`channel: web_search_only`) |
| `nations.jsonl` | 326 | nation crosswalk stub (Phase 0) — see "Nation crosswalk" below |

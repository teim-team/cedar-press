...
1. **The Cedar identity block comes first, in this order, in every table:**
   ...
   same three columns. `entity_class` is the REGISTER's class, written at build time
   from the uid: never `entity_class_scope` (a class-level scope, its own `_basis`
   says so), never `Native_Party_Type` (the party's type as a source published it),
   never `cedar_native_entity_class` (the organization's own kind). Those stay as
   what they are, after the identity block. Where a table today carries the name
   or class under another header (`native_party_canonical_name`, `owner_hub_name`,
   `cedar_spine_canonical_name`, `entity_type`), keep that column if it means
   something different and ADD the canonical ones at the front; where it means the
   same thing, rename it.
...
3. **Four tiers of column.** A: what the record says (the customer file). B:
   ...
   **C: the CONTROLS, which ship, labelled**: the flags that fence money and grain and
   without which a customer cannot reproduce Cedar's own totals: `attributed_flag`
   and `attribution_status` (funding ships unattributed rows on purpose; the totals
   count `attributed_flag = 1` only), `fy_partial_flag` (a partial fiscal year is not
   a year), `duplicate_status` and `subaward_exceeds_prime_flag` (the subaward
   money fence: $57.02B unfiltered, $34.91B after both), `action_date_precedes_ffata_flag`
   (49 filings whose dates are known anomalies), `attribution_withdrawn` and its
   reason (a withdrawn attribution stays in the file as a correction and its spend is
   not Native lobbying), `is_event_primary_row`, `document_role` and
   `n_participant_rows_for_event` (2,313 consultation documents fan out to 11,402
   participant rows; the count is `SUM(is_event_primary_row)`), the NAGPRA
   `n_*_named` / `n_*_resolved` pairs (51,338 named relationships, 47,688 resolved:
   the gap is real uncertainty and must ship as such), and supersession on
   filings. D: pipeline bookkeeping (`built_date`, `fetched_date`, `retrieved_date`,
   ...
   `population_basis`, `floor_basis_field`, `pre_2000_flag`, the `geo_*` dominance
   shares and ambiguity flags, `deflator_factor_2025`, `inflation_base_year`). Tier D
   stays in the workspace and out of the customer file. The `*_real2025` columns
   are a judgment call: keep them beside their nominal column, with the deflator
   and base year dropped and stated once in the collection's documentation.
4. **Never drop a column because it is empty or constant in this release.** A loan
   field or `related_asset_ids` is legitimately empty now and populated by the next
   refresh, and `1137_customer_dataset_combine.py` keeps all-empty columns on
   purpose so a buyer diffing two deliveries does not watch columns appear and
   vanish. A column leaves the customer file only by an explicit semantic
   deprecation recorded here, and a blank in a ten-row sample proves nothing.
5. **Identifiers stay.** `cage_code` and the `*_cage` columns are the settled first
   identifier (CAGE persists across the DUNS-to-UEI transition) and join to
   historical awards, SAM and the identifier ledger; keep them. The publication
   carve-out for individually Native-owned firms withholds a UEI or CAGE only where
   `firm_legal_name_is_person` is 1 or UNKNOWN and no consent is recorded: mask
   those cells on those rows, never the column.
6. **Order within a table:** identity block → record id → what happened (date,
   year, kind, title or description) → who else (client, registrant, awardee,
   counterparty, institution) → money and its basis → status and the controls →
   source link → tier-B provenance. Nothing after the source link should be
   something a reader needs to understand the row.
...
Controls that ship (tier C): `attributed_flag`, `attribution_status` (the file ships unattributed rows on purpose, and the totals count attributed ones only), `fy_partial_flag` (FY2026 is partial, and so was the FY2023 vintage). Keep the loan columns even where empty.
Drop (tier D, 19): `attribution_source_line`, `attribution_rule`, `exclusion_reason`, `exclusion_source_line`, `exclusion_rule`, `ak_flag`, `excluded_flag`, `credit_instrument_flag`, `business_types_code`, `deflator_factor_2025`, `inflation_base_year`, `population_basis`, `state_agreement`, `source_archive_stamp`, `fetched_date`, `attribution_basis`, `source_vintage_basis`, `business_types_description_normalized`, `business_types_description_normalized_basis`, `recipient_duns` (retired identifier), and the `geo_*` dominance shares, ambiguity flags, key tier, key basis and build date; keep the two county names and FIPS.
Note: the sample has no source URL column; the site builds the USAspending award page from `assistance_award_unique_key`. Write `source_url` into the file so the file is self-sufficient.
...
Controls that ship (tier C): `is_event_primary_row` (one row per event carries it; `SUM(is_event_primary_row)` is the consultation count, and a customer counting rows instead overstates by about 4.9×), `document_role` (a notice announcing a consultation is not a report of one that happened), `n_participant_rows_for_event`.
Drop (tier D, 10): `channel` (constant by construction), `fetched_date`, `built_date`, `nagpra_notice_overlap`, `nagpra_bridge_overlap`, `nagpra_coverage_window`, `event_date_basis`, `event_date_source_quote`, `location_basis`, `location_source_quote`.
...
Controls that ship (tier C): every `n_*_named` / `n_*_resolved` pair, `n_parties_named`, `n_entities_resolved`. The bridge names 51,338 party relationships and resolves 47,688; the 3,650 unresolved are real associations whose identity is unknown, and a file that shows only the resolved uids beside their names would present the subset as the whole. Better still, publish the unresolved names as published beside each role's ids.
Drop (tier D, 12): `notice_title_form` (the same fact as `notice_type`), `institution_primary`, `institution_count`, `institution_name_basis`, `mni_basis`, `mni_statement_count`, `removal_location_basis`, `has_resolved_entity`, `parse_template`, `spans_found`, `parent_dataset`, `artifact_mtime`, `institution_split_flag`, `institution_split_basis`; keep `fetched_date` only if it is ever populated.
...
Controls that ship (tier C): `attribution_withdrawn` and `attribution_withdrawn_reason`. A withdrawn attribution is a real LDA filing that stays in the file with its Cedar link withdrawn; the flag is one leg of the money fence that keeps its spend out of Native lobbying totals, and the reason is the correction's evidence. `attribution_withdrawn_entity_id` and `_date` can stay in the workspace.
Drop (tier D, 8): `entity_id` (duplicates `cedar_uid`), `entity_state` (the register has it), `matched_alias`, `pull_keyword`, `org_type_barred`, `org_type_reason`, `filing_url_original` (keep one URL), `attribution_withdrawn_entity_id`, `attribution_withdrawn_date`.
...
Controls that ship (tier C): `duplicate_status` and `subaward_exceeds_prime_flag` together (the money fence: $57.02B unfiltered, $34.91B after both; a row that exceeds its prime is a real filing that must not be summed), `action_date_precedes_ffata_flag` (49 filings whose action dates precede FFATA and are known filer anomalies; without the flag a year grouping publishes a phantom pre-2010 series). Keep the four `*_cage` columns (identifiers; carve-out masking only).
Drop (tier D, 30): `fetched_date`, `pre_2000_flag`, `floor_basis_field`, `source_population`, `promoted_date`, `deflator_factor_2025`, `inflation_base_year`, `subaward_sam_report_month`, `subaward_sam_report_last_modified_date`, `subaward_sam_report_id_basis`, `subaward_source_record_id_basis`, and all twenty-one `geo_*` except `geo_subawardee_city`, `geo_subawardee_state_code`, `geo_subawardee_county_name`.
...
### need / need_enterprises (65 columns)
...
Keep `related_asset_ids` even while empty (rule 4).
Drop (tier D, 8): `deflator_factor_2025`, `inflation_base_year`, `land_status_basis`, `fetched_date`, `built_date`, `cedar_uid_basis`, `record_scope_basis`, `entity_attribution_status`, `entity_attribution_basis`.
...
1. The customer files are written by `code/1137_customer_dataset_combine.py` into
   `dist/customer/<collection>.csv`; that is where the column order and the tier-D
   removals land. `770_sample_extracts.py` (samples) and
   `1135_full_dataset_review_bundle.py` (`dist/review/`) follow so the previews and
   the review bundle show the same header. A change to 770 and 1135 alone changes
   nothing a customer downloads.
2. Add `entity_class` (and `canonical_name` where absent) from the register at write
   ...
   that can drift; never substitute a scope or a source's own type column.
...
4. Re-run `scripts/import_cedar_manifest.py` (which now strikes any sample that
   publishes a withheld field), then `node scripts/derive-explore.mjs`; the
   contracts re-derive, and `data/cedar/codebook.json` (the labels and meanings the
   viewer shows) and its `default_columns` need renaming to match. The site's tests
   name every declared column that no longer exists.
5. Publish the column list per table in the collection's documentation, one line a
   column, with the tier-B provenance and the tier-C controls explained
   (`docs/DATASET_CODEBOOK.md` is that list, generated from the codebook).

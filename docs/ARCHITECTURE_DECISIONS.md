...
`code/1149_codebook_money_fed.py`. Build logs
...
| `code/1145_*`, `code/1148_*`, `code/1149_*` | NEW, each claimed atomically via `1050_preflight.py claim` and written into the stub it created |
...
| `data/clean/codebook/{10e,10f,11e,11f,11g,11h,11i,11j,11k}_*.csv` | NEW fragments; `codebook_master.csv` rebuilt through `cedar_codebook.build()`, which refuses to shrink |
...
already reached by the existing patterns in `500`, so none of the nine tables is

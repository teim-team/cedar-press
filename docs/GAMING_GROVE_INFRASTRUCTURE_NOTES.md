...
for the repository split. Consolidated 2026-09-25 onto Cedar PR #122 as branch
`claude/gaming-consumer-hardening`; the earlier `claude/gaming-grove-consumer`
and `75a2f1b` branches are superseded and not for merge. **Lumecon-data owns the Gaming data.**
The producers, candidate build, leak gate, data contract and release/catalog
build moved to Lumecon-data PR #9 (`codex/collection-release-closure`) and its Gaming hardening branch `claude/gaming-release-hardening`, with their
...
6. The collection manifest fetched from the Lumecon API hashes to
   ...
   A `partitioned_components` declaration (bounded parts, e.g. payments)

...
| `CEDAR-NEST-nnnnnn-CC`, the enterprise sub-hub register (4,798 enterprises since the owner's dataset became a builder input, ADR-034; 1,512 rows carry a stated ownership tie and 3,286 an affiliation that is not an ownership claim, `docs/methodology/need.md`; keyed to `owner_hub_cedar_uid`) | `data/spine/cedar_need_id_register.csv`; ADR-020; ADR-034 | An enterprise tied to a nation, ANC or NHO is a commercial operating company linked to a Cedar entity, and NEED is the business register's largest seed. The seed keeps each row's evidenced relationship class: an ownership relationship is written only where a source stated ownership, and an affiliation stays an affiliation, so the 3,286 affiliation rows do not become ownership by being seeded. The NEED id is retained as a source-scoped key beside the `CB-` id, as ADR-030 kept `facility_id` beside `cedar_place_id`; nothing is re-keyed by deletion. ADR-020's rule that a NEED id may never appear where a `cedar_uid` is expected carries over to `CB-` unchanged. |

...
|---|---|---|
| `Individually Native-owned business`, an entity class in `cedar_identity_register.csv` (45 rows, handle prefix `CEDAR-ENT-`, self-parented, never rolls up, publication withheld absent consent) | `code/cedar_domain.py` `INDIVIDUAL_NATIVE_CLASS`; `docs/INDIVIDUAL_NATIVE_CLASS_PROPOSAL.md` | These are businesses by the reviewer's definition and belong in the business register. Their uids are **not deleted and not reused** (the standard's rule and the reviewer's rule agree). Each gets a `CB-` id, and the register records the equivalence. Whether the class stays open for new mints is the owner's call; the decision's default is that a privately owned Native business is a `CB-` first. |
| `CEDAR-NEST-nnnnnn-CC`, the enterprise sub-hub register (1,375 enterprises, 1,610 bindings, keyed to `owner_hub_cedar_uid`) | `data/spine/cedar_nest_id_register.csv`; ADR-020 | An enterprise a nation, ANC or NHO owns is exactly a commercial operating company linked to a Cedar entity by an ownership relationship. NEST is the business register's largest seed. The NEST id is retained as a source-scoped key beside the `CB-` id, as ADR-030 kept `facility_id` beside `cedar_place_id`; nothing is re-keyed by deletion. ADR-020's rule that a NEST id may never appear where a `cedar_uid` is expected carries over to `CB-` unchanged. |
...
| The identifier ledger (one row per `identifier_type`, `identifier`, entity; UEI, CAGE, EIN today; state ids and licence numbers welcome) | `cedar_identifier_ledger_final.csv`; standard §3 | The ledger's shape is the right one and it gains a second subject column: an external identifier binds to a `CB-` id or to a `CE-` uid, never to both on one row. A UEI bound to a business rolls up to the entity through the ownership relationship, which is what ADR-008 asked for and could not have without a legal-person key. |
...
5. **Registrations are attributes.** UEI, CAGE, EIN, DUNS (internal only), state corporation numbers, TERO and tribal vendor numbers, SBA and PPP identifiers bind to the business in the identifier ledger. None of them is the business's identity, and a business with no external identifier is still a business.
6. **Aliases, not new ids, for name changes.** `business_alias` with `valid_from`, `valid_to`, `source` and `alias_type` (legal, trade, filing variant, source spelling). Successors, mergers and newly created entities get new ids and a typed edge (`SUCCESSOR_OF`, `MERGED_INTO`).
...
- The seed: NEST enterprises, the 45 individually owned entities, the certified-firm directories, the contracting awardees and subawardees with a UEI or CAGE, deal parties, and lobbying registrants (ADR-008's population). Each seed keeps its source key beside the new id.
...
- The field map declares `cedar_business_id` on the datasets where a business is a party: Contractors (the awardee), Subcontracting (prime and sub), Deals (buyer, seller, target), NEST (the enterprise; `enterprise_id` stays as the source key), Owned (the business; `business_entity_id` retires to it under the retirement rule), Lobbying (the registrant), and Federal Funding where the recipient is a business rather than an entity. The plural block gains no business column: a record's Native entities stay in `cedar_uids`, and a party list of businesses is its own aligned column.
...
- The publication rule for individually Native-owned businesses (`may_publish_individual_native_field`) applies to the business id as it applies to the name: a `CB-` id on a withheld row would let a reader link the withheld business across datasets, which is what withholding exists to prevent. Recommended, and for the owner to confirm: the id is withheld with the name until consent is recorded.

## Two things the owner still decides

1. **Check characters.** The reviewer's format is `CB-0000001`, a plain sequence. The standard's §0 argues, with measurements, that an identifier a customer transcribes needs check characters (two of them catch every single substitution and every adjacent transposition on the live register), and it excludes `O`, `I`, `L` and `U` from the alphabet for that reason. The recommendation of this note is to keep the reviewer's semantics (a meaningless sequence, nothing encoded) and the standard's mechanics (two check characters over the same alphabet), so a business id is transcribed as safely as an entity id. A plain sequence is the reviewer's decision as written; the check characters are a suggestion.
2. **The individually owned class.** Whether `Individually Native-owned business` stays open as an entity class after the 45 gain business ids, or closes to new mints with every future privately owned firm a `CB-` only. The reviewer's rule points to the second; the publication machinery built around the class keeps working either way, because it keys on the class of the row, not on which register minted the id.

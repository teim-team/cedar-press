...
**Today:** One payment or distribution of natural-resource revenue, with its measurement status and aggregation level.

**When the specification is applied:** The same: one resource-related financial event with an explicit revenue type, measurement status and aggregation level; aggregates never assigned to a tribe.
...
`resource_revenue_event_id` names the event; `source_system` and `source_record_id` name the record in its source. `beneficiary_entity_id`, `payer_entity_id` and `operator_entity_id` are the other parties' identifiers, which are Cedar IDs only where the party is a Native entity. A cross-reference to the Federal Funding award (`related_funding_award_id`) is owed where a reliable identifier exists.
...
The opening block of every row is `cedar_uid`, `cedar_entity_name`, `cedar_entity_type` and `cedar_entity_role`. `cedar_entity_role` is `recipient`. `beneficiary_entity_id` names the beneficiary where it differs from the recipient (a distribution to allottees through a tribe, say), with `beneficiary_note` explaining. `attribution_status` says whether the row is keyed to an entity, to an aggregate or unresolved, so a blank `cedar_uid` has a stated reason.
...
The approved header, in order (38 columns, of which 1 are owed and marked so). Data types are read off the ten-row sample the site serves; identifiers are text and keep leading zeros.
...
| 6 | `attribution_status` (was `entity_attribution_status`) | Attribution status | Whether the row is keyed to a Native entity, to an aggregate, or unresolved. A blank Cedar ID has a stated reason here. | text | the source states none, or not applicable to this row |
| 7 | `beneficiary_entity_id` | Beneficiary ID | The identifier of the beneficiary where it differs from the recipient. | identifier, as text | the source states none, or not applicable to this row |
| 8 | `beneficiary_entity_name` | Beneficiary | Who the payment is for, where different from the recipient. | text | the source states none, or not applicable to this row |
| 9 | `beneficiary_note` | Beneficiary note | How the source describes the beneficiary. | text | the source states none, or not applicable to this row |
...
| 11 | `payer_entity_name` | Payer | Who made the payment (a federal office, a state). | text | the source states none, or not applicable to this row |
...
| 13 | `operator_entity_name` | Operator | The related operator's name, where a source supports one. | text | the source states none, or not applicable to this row |
...
| 24 | `measurement_status` | Actual or estimated | Whether the amount is an actual payment or an estimate. | text | the source states none, or not applicable to this row |
| 25 | `amount_usd` | Amount | Dollars paid. The sign column says what a negative means. | amount in US dollars, as recorded (no rounding; negative where the source records a reduction) | the source reports no amount; never zero |
| 26 | `amount_usd_real2025` | Amount in 2025 dollars | The same amount adjusted to 2025 dollars. | amount in US dollars, as recorded (no rounding; negative where the source records a reduction) | the source reports no amount; never zero |
| 27 | `amount_sign_meaning` | What the sign means | How to read a negative amount (a correction, a recoupment). | amount in US dollars, as recorded (no rounding; negative where the source records a reduction) | the source reports no amount; never zero |
| 28 | `aggregation_level` | Aggregation level | Whether the amount is specific to the entity, a regional aggregate or a countrywide aggregate. An aggregate is never assigned to one tribe. | text | the source states none, or not applicable to this row |
...
| 30 | `geography_note` | Place | What the source says about where the revenue arose. | text | the source states none, or not applicable to this row |
| 31 | `allocation_formula` | Allocation formula | The rule that determined the amount, where the source states it. | text | the source states none, or not applicable to this row |
| 32 | `allocation_formula_effective_start` | Allocation rule effective from | When the allocation rule took effect. | date (YYYY-MM-DD) | the source states no date |
| 33 | `allocation_formula_effective_end` | Allocation rule effective to | When the allocation rule ceased to apply. | date (YYYY-MM-DD) | the source states no date |
| 34 | `allocation_formula_source_url` | Formula source | Where that rule is published. | web address | the source states none, or not applicable to this row |
| 35 | `related_funding_award_id` | related funding award id | The Federal Funding award this event also appears as, where a reliable identifier exists. | — | owed: not in the file until the terminal builds it |
...
| 37 | `source_record_id` | Source record ID | The record's identifier in that source. | list, separated by | | the source states none, or not applicable to this row |
| 38 | `source_url` | Source | Where the payment is recorded. | web address | the source states none, or not applicable to this row |
...
- `related_funding_award_id` (pending:cross-reference): The Federal Funding award this event also appears as, where a reliable identifier exists.

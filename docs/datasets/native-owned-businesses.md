...
<!-- CEDAR:COVERAGE-MEASURED collection=native-owned-businesses START -->
...
**Status: READY** — 6 customer tables, shelf `pro`, measured 2026-09-02 by `518`.
...
|---|---:|---|---|
| `individual_native_exclusion_pairs.csv` | 5 | `ruled_date` | 2026 |
| `individual_native_firm_contracts.csv` | 324 | `fiscal_year` | 2000–2022 · **thin: 2001** |
| `individual_native_firm_contracts_published.csv` | 613 | — | no dated column |
| `individual_native_firm_register.csv` | 45 | `ruled_date` | 2026 |
| `individual_native_ownership_verification.csv` | 335 | — | no dated column |
| `individual_native_prior_rulings.csv` *(internal-by-decision)* | 45 | `ruled_date` | 2026 |
| `individual_native_verification_candidates.csv` | 335 | — | no dated column |

<!-- CEDAR:COVERAGE-MEASURED collection=native-owned-businesses END -->

## COVERAGE — per source, years upstream vs years Cedar holds

*Authored 2026-09-01 by workstream DOCS. The `Years Cedar holds` half is in the
measured block above and is deliberately not repeated here, so the two cannot
disagree.*

**This dataset's coverage question is not primarily a year range**, and saying so
is the honest answer rather than a dodge. A tribal or state Native-business
registry publishes **who is certified now**. It is a roster, not a series: most
of these publishers do not archive last year's list, so there is no upstream
year range to be short of. Section *COVERAGE AGAINST THE MASTER ENTITY LIST*
below is the coverage measure that actually binds — how much of the spine has
been checked — and it is the one to quote.

| Source | Years upstream | Gap |
|---|---|---|
| Tribal and inter-tribal vendor / certification registries | **A CURRENT SNAPSHOT, not a series.** No publisher in the harvest is known to archive superseded lists. | **There is no year gap to close, and there is an irreversible one accruing:** every list not captured this quarter is gone. Keep vintages the way `06_nonprofit`'s BMF vintages are kept. |
| State Native / minority business certification directories | **NOT PROBED.** No upstream probe for archive depth is recorded in this repo. | Unquantified. |
| SBA disaggregated contracting-ownership data | **Begins 2020.** CICD, 2022-12-21, verbatim: *"In 2020, the SBA began releasing disaggregated ownership data of businesses involved in federal contracting."* The pre-2000 goaling reports carry small business, SDB, 8(a), women-owned and HUBZone — **no Native category**. | **No pre-2020 Native-ownership series exists at this source.** Not a route to history. |
| FPDS Native business-type flags | Present in the ATOM schema on FY1985 records and carrying `false` on **all** of them, while `isSmallBusiness` is true on 44% of the same sample — a per-attribute absence, not a blank record (`code/563`, sampled live 2026-09-01). | **Native identification did not exist in the federal record before roughly FY2000.** This is the same finding that bounds `02_contracting.md`, and it bounds any attempt to date this roster backwards. |

**The verdict.** The time dimension nobody has used yet — noted at the end of
this file — is the *capture* dimension: Cedar's own successive harvests are the
only longitudinal record of Native business certification that will exist,
because the publishers overwrite. That makes vintage retention a build rule

...
**Status: READY** — 54 customer tables, shelf `grove`, measured 2026-09-02 by `518`.

No open contract point. Next action: **maintain**.

| contract point | state |
|---|---|
| C1 grain stated | 54/54 |
| C2 validated primary key | 54/54 |
| C3 literal duplicates | clean |
| C4 identity path | 92% keyed |
| C5 row conservation | 1/54 |
...
> **PROMOTED 2026-09-01 (workstream INT-2). All four families now SHIP.**
> `code/585_factcheck_nigc_keys.py` re-derived every tribe key before
> anything moved and 20 of 532 did not survive; `code/586_promote_nigc_gaming.py`
> wrote six tables and their codebook blocks, and all six reached
> `dist/cedar_press.db`:
> `nigc_enforcement_actions` 362 · `nigc_indian_lands_opinions` 102 ·
> `nigc_game_classification_opinions` 122 ·
> `nigc_management_contract_approvals` 68 · `nigc_document_surface` 7,930 ·
> `nigc_action_parties` 386 (the ADR-010 party bridge).
> **The surface is an INDEX, not an instrument table** — 7,930
> (category, document) memberships over 4,071 distinct documents — and must
> never be summed against `nigc_ordinances` (1,155) or
> `nigc_declination_letters` (327). What it MEASURES is that NIGC's index now
> carries 1,162 and 329 of those two, i.e. **+7 and +2**, and those two
> numbers are the refresh signal.
...
| OSHA ITA 300A + Census LODES | establishment-level injury filings and workplace jobs | `gaming_employment_observations` 3,421 | 2008–2026 (OSHA CY2016–2025) | **CURRENT to source, and the review backlog is now adjudicated.** `review/osha_gambling_unresolved_2026-08-26.csv` holds 4,560 filings 157 did not attach. 2,551 are `blocked_commercial` (Boyd, Caesars, MGM, IGT, Station, state lotteries) and 1,849 `unresolved` with no spine match — both sets are overwhelmingly NON-TRIBAL and stay refused; NAICS 7132/721120 is the gambling industry, not the tribal gambling industry. **160 were neither, and those are the ones nobody had looked at.** `583` adjudicated them 2026-09-01: **79 rows over 19 tribes promoted**, 81 held with a per-row reason, in `review/osha_gambling_adjudicated_2026-09-01.csv`. The separate 711-establishment hold from 2026-08-07 is also ruled: 115 further rows promoted, every one of the 711 given a written decision. See §1D-a. |
| DOL Form 5500 (retirement) | plan sponsor, active participants — **NOT a headcount**, see below | **MERGED** — 1,956 rows in `gaming_employment_observations`; 2,046 staged, 90 refused | 2009–2025 | **PROMOTED, AND BOTH RULINGS ARE RESOLVED.** The "promotion owed" line was six days stale: the merge happened 2026-08-26 and both measurement types were already live in `cedar_domain`. What had never run was the fact-check. See §1D-a. |
...
#### 1D-a. The labour surface, and the two rulings that were said to block it

*Measured and written 2026-09-01 by workstream INT-1.  Cedar holds **no NLRB,
QCEW, BLS wage, Wage-and-Hour, EEOC or OLMS data at all** — Form 5500 and OSHA
ITA are the entire labour surface, and this section is all of it.*

**The "PROMOTION OWED" line above was stale by six days.** Both staged files
were already merged on 2026-08-26 and both measurement types were already live
in `cedar_domain.MeasurementType` with full definitions. What had never run was
the fact-check over what promotion wrote. The two rulings are recorded here
because they are project vocabulary now, not because anything was waiting on
them.

**RULING 1 — `FORM5500_ACTIVE_PARTICIPANTS` is a measurement type, and it is
not employment.** A row is DOL's `TOT_ACTIVE_PARTCP_CNT`: the number of people
enrolled in one benefit plan sponsored by a tribal employer in a gaming NAICS,
for one plan year. It EXCLUDES employees who never enrolled or who sit below
the plan's age/service threshold, and it INCLUDES separated employees who still
hold a balance. **The two errors do not cancel and their net sign is not
stable** — against SEC-overlapping tribe-years the ratio to full-time headcount
is 1.65 (largest retirement plan), 1.19 (largest welfare plan) and 0.79 (against
a study's total employment). None of those is a calibration factor. It is a
defensible lower-bound-ish proxy for the size of the enterprise and a usable
CHANGE series (slope ~0.63 against SEC full-time counts, R² 0.86, 11 pairs, 2
entities). It is not a level, and it never promotes to an active headcount —
which is what `measurement_type_status = NEVER_PROMOTES_TO_ACTIVE` says on every
such row. A sponsor's several plans are never summed; the largest is taken,
because participants overlap across a retirement and a welfare plan.

**WHAT A BUYER WOULD GET WRONG, NOW STATED IN THE CODEBOOK:** adding
`FORM5500_ACTIVE_PARTICIPANTS` to `OSHA_TRIBE_LEVEL_REPORTED` for the same
tribe-year counts the same casino floor twice. Over the 192 tribe-years carrying
both, the **median ratio is 1.03** — they are two measurements of the same
people, not two workforces. Adding `OSHA_TRIBE_LEVEL_REPORTED` to
`OSHA_ESTABLISHMENT_REPORTED` double-counts too: **332 tribe-level rows are the
same 300A filing** as a facility-grain row, and `already_facility_attached = 1`
marks every one. `LODES_BLOCK_WORKPLACE_JOBS` sums with nothing — a census block
holds the casino and its neighbours. Only distinct
`OSHA_ESTABLISHMENT_REPORTED` establishments within one year may be added, and
even that total is "employment at the establishments that filed", never total
tribal gaming employment.

**RULING 2 — the employment table ADMITS tribe-level rows with no
`facility_id`.** 2,556 of 3,306 rows (77.3%) carry none, by design. A Form 5500
is filed by a PLAN SPONSOR — the tribe or its enterprise — and names no casino;
an OSHA 300A names an establishment Cedar can sometimes tie to an owning tribe
but not to one of its properties; a LODES figure is a census block. Requiring a
`facility_id` would mean inventing an attribution the filing does not make, and
where a tribe runs several casinos there is no non-arbitrary way to pick one.
Blank beats confident-wrong, so the row is admitted at the grain the source
states, and `entity_level` records which grain that is (`facility` 749, `tribe`
2,546, blank on 11 legacy NEPA rows) while `geographic_level` carries the
source's own words for what the number is measured over. **Consequence for a
consumer: a join on `facility_id` is not a join on this table, it is a join on
the 22.7% of it that is facility-grain.** The contract in `512` already reads
*"one row per employment observation at one geographic level"* with
`facility_id: many`, which is consistent with the ruling; GRAIN-WS3 owns making
the admission explicit there; the suggested wording, with the measured counts,
is in `review/grain_note_gaming_employment_2026-09-01.md` (INT-1 could not
reach that agent directly and does not edit 512).

**THE FACT-CHECK FOUND ONE LIVE MISATTRIBUTION, AND IT IS THE UMATILLA SHAPE
AGAIN.** Script `157`'s own header records that unguarded containment resolved
`CAESARS PALACE LAS VEGAS`, `BALLY'S LAS VEGAS` and `Circus Circus Las Vegas` to
**Las Vegas** — the Las Vegas Paiute Tribe, `TRBF-LSVGAS-00` — and 157 guarded
against it. **Script `156` took its Form 5500 rows from 4wheeler's resolver,
which has no such guard, and the same defect reached `data/clean/`.**
`TRBF-LSVGAS-00` carried 25 Form 5500 rows; six are the tribe and nineteen were
four unrelated commercial Las Vegas employers:

| sponsor as filed | rows | participants |
|---|---:|---|
| WESTGATE LAS VEGAS RESORT & CASINO | 11 | 454–809 |
| GAMING VENTURES OF LAS VEGAS, INC(.) | 6 | 105–144 |
| BARDEN NEVADA GAMING, LLC DBA FITZGERALDS LAS VEGAS | 1 | 275 |
| LAS VEGAS GAMING, INC. | 1 | 34 |
| **LAS VEGAS PAIUTE TRIBE** (the tribe) | **6** | **204–343** |

The tribe's own filings run 204–343 and Westgate's run 454–809, so left in place
a buyer reads the tribe's workforce as tripling and then collapsing. The 19 were
removed by `583` into `review/gaming_employment_lsvgas_removed_2026-09-01.csv`,
for the same reason the other 71 staged rows were never promoted — Hawaiian
Gardens Casino (a commercial LA-county card club keyed to a Native Hawaiian
organisation), Prairie Meadows (an Iowa county racino keyed to Prairie Band
Potawatomi), Casa Blanca Resort NV (keyed to a BIE school in Arizona), Double
Eagle Cripple Creek, Gaming Entertainment (Delaware) LLC (keyed to the Delaware
Nation of Oklahoma).

**THE TEST THAT ISOLATED THEM**, re-runnable on any future labour source: a row
is EXPOSED when the sponsor text carries no governmental word (tribe / nation /
band / community / pueblo / rancheria / indians / village) AND no facility in
`gaming_facilities.csv` belonging to that tribe has its distinctive name tokens
present in the text. Over all 1,975 promoted Form 5500 rows it returned 102 over
18 sponsors; 83 are correct enterprise brands Cedar's facility table simply
spells differently (`GILA RIVER GAMING ENTERPRISES`, `PCI GAMING AUTHORITY`,
`SENECA GAMING CORPORATION`, `RED LAKE GAMING ENTERPRISES`, `OSAGE CASINOS`) and
19 are the block above. Full output:
`review/gaming_employment_exposure_2026-09-01.csv`. **The test is a prompt, not
a verdict**, which is why it is written to review rather than acted on
wholesale.

**Everything else the keying check ran cleanly.** All 239 distinct `tribe_id`
values in the table resolve in `data/spine/cedar_entity_spine.csv` — zero
orphans, zero canonical-name disagreements.
`262_repair_form5500_tribe_attribution` had already caught the other
tribe-to-tribe confusions the 4wheeler resolver produced: Golden Eagle →
Kickapoo of Kansas, Eagle Mountain → Tule River, Lucky Eagle → Chehalis, Four
Winds → Pokagon (from a BIE school in North Dakota), Prairie Wind → Oglala,
Prairie Knights → Standing Rock, Sac & Fox Casino → Sac & Fox of Missouri,
Meskwaki → Sac & Fox of the Mississippi, Seminole Nation of Oklahoma and United
Keetoowah Band each to their own id — 133 rows. **It caught them because they
all failed a state check. Las Vegas passed one, which is exactly why it
survived**, and why the corroborator here is a governmental word rather than a
state.

**FOUR ROWS ARE FLAGGED, NOT REMOVED.** `SITNASUAK NATIVE CORPORATION` (3) and
`BERING STRAITS DEVELOPMENT COMPANY` (1) are keyed to the right entity, but
neither sponsor is a gaming employer and Cedar holds no gaming facility for
either. They carry `SPONSOR_IS_NOT_A_GAMING_EMPLOYER_NO_CEDAR_FACILITY`. The
attribution is right and the filing is real, so removing them would be a
different error. `CIRI LAKESIDE GAMING INVESTORS LLC` (1, NV) is kept unflagged:
CIRI is an ANC with a real Nevada gaming investment vehicle and the sponsor name
is exact and distinctive.

**MAGNITUDES ARE SANE, AND THE OUTLIERS ARE COVERAGE, NOT ERROR.** Median
Form 5500 / OSHA ratio across the 192 overlapping tribe-years is **1.03**.
Nineteen tribe-years exceed 3× — worst are Kalispel 2022 (×52), Kalispel 2024
(×43), Sault Ste. Marie 2025 (×34), Ho-Chunk 2016 (×20). Every one is the
documented ITA gap: the 5500 covers the whole plan-sponsoring enterprise, ITA
only the establishments that chose to file. **An establishment absent from ITA
did not file; it does not have zero employees and it does not have zero
injuries.** Thirteen tribe-years show more OSHA establishments than Cedar
facilities — mostly Gila River, whose gaming enterprise files a 300A for a
corporate HR centre and a distribution centre that are not casinos.
`TRBF-MCHPDA-00` files a 300A establishment and has **zero rows in
`gaming_facilities.csv`**, which is a gap in the facilities table, not in this
one.

**THE 160 UNADJUDICATED OSHA HOLDS: 79 promoted, 81 held.** Both promotion rules
require evidence that is in the filing or in Cedar's own curated facility table,
never outside knowledge.

* **O1** — every distinctive token of a spine entity's name is present in the
  filing, the filing carries a governmental word, the state agrees, and no other
  spine entity in that state matches. 28 rows. This is what reaches
  `Mississippi Band of Choctaw Indinas` → Pearl River Resort (the typo is
  OSHA's), `Rincon Band of Luiseno Indians`, `Kalispel Tribal Economic
  Authority`, `Warm Springs Casino Enterprises`, `Cherokee Nation Entertainment,
  LLC` → Will Rogers Downs.
* **O2** — the filed name carries the same distinctive tokens as a
  `gaming_facilities.facility_name` belonging to exactly one tribe in that
  state. 50 rows: Viejas, Northern Quest, Win-River, Potawatomi Bingo, Kiowa
  Casino, Two Kings, Quinault Beach, Harrah's Cherokee, The Oneida Hotel, three
  St. Croix properties. This is the same pass-B evidence 157 used for 345 of its
  own 502 rows.
* **O2b** — the same nationally, where the filing state differs from the spine
  state, recorded with `state_mismatch_flag = 1` rather than silently accepted.
  1 row: Quechan / Paradise Casino, filed in Yuma AZ against a spine state of CA
  — a border property, the same shape as Rosebud filing from NE and Catawba
  from NC.

Two guards earn their place and both were added after a false positive got
through in testing:

* **`entity_class` must be able to own a casino** — `Federally recognized
  tribe`, `Federal-level constituency entity`, `Federally recognized Alaska
  Native Village`, which is not an opinion but the set of classes that actually
  own a row in `gaming_facilities.csv` (739 / 43 / 3). Without it the subset test
  hands *Yakama Nation Legends Casino Hotel* to `BIE-YKMNTN-00`, the Yakama
  Nation **tribal school**, and *Harrah's Cherokee* to `CEDAR-ENT-000061`, an
  individually Native-owned business called Cherokee Enterprises Inc — both beat
  the real tribe on uniqueness because the real tribe's spine name carries a
  token (*Confederated* Yakama, *Eastern* Cherokee) that the filing does not
  print.
* **O2b requires a governmental word.** Without it *Double Eagle Hotel & Casino*,
  Cripple Creek CO, matches Cedar's *Double Eagle Casino* in Chewelah WA and
  lands a commercial Colorado property on the Spokane Tribe of Indians.

**The 81 still held are held for a stated reason, and it is the right one.** Most
are commercial (Sahara / Flamingo / Paris / Harrah's / OYO / SLS / Westgate Las
Vegas, Prairie Meadows, Hawaiian Gardens, Trop Greenville, Zia Park, Ocean
Downs, San Juan Marriott, Courtyard Houma). Five are real ambiguities worth
naming, because each would be settled by one small specific thing rather than by
more matching:

| held | rows | what would settle it |
|---|---:|---|
| `Squaxin Island Gaming dba Little Creek Casino Resort` WA | 3 | the filed string carries no governmental word and does not token-match `Little Creek Casino Resort` on its own. An alias row on the facility |
| `Yakama Nation Legends Casino Hotel` WA | 3 | Cedar's facility is `Legends Casino Hotel`; the filing prepends the nation. A facility alias, or `Yakama` as a spine alias of `Confederated Yakama` |
| `Ho-Chunk Inc.` / `Warhorse Casino` Lincoln & Omaha NE | 7 | **Ho-Chunk Inc is the Winnebago Tribe of Nebraska's corporation, NOT the Ho-Chunk Nation of Wisconsin.** A name match gets this exactly backwards. Needs a ruling, not a rule |
| `Mohegan Sun Pocono` PA / `Mohegan Casino Las Vegas at Virgin Hotels` NV | 2 | Mohegan-managed properties off Mohegan land. Whether a management contract is a Cedar attribution is an attribution question, not a schema one |
| `Eagle Mountian Casino` (sic) CA | 2 | a source typo; Cedar holds `Eagle Mountain Casino`. A typo-tolerant alias on that facility, never a fuzzy matcher |


**THE 711-ESTABLISHMENT HOLD IS ALSO RULED, AND ITS HEADLINE WAS 58% STALE.**
`review/employment_osha_unmatched_2026-08-07.csv` held **711 establishments /
1,879 filings** with `YOUR_RULING` blank on all 711, every one on the same
stated reason: *"shares a distinctive token with a Cedar property but no exact
name+state match."* **Holding on that reason was right** — a shared token is
the Umatilla defect and `docs/ENTITY_MATCH_RULES.md` rule 1 refuses it. `pearl`
is not why Pearl River Resort belongs to the Mississippi Band of Choctaw
Indians. But the file is a 2026-08-07 snapshot and two later passes moved under
it:

| | establishments | filings |
|---|---:|---:|
| already promoted since the snapshot | 66 | 194 |
| already ruled `blocked_commercial` by 157 | 344 | 965 |
| **genuinely still open** | **301** | **720** |

`589_adjudicate_osha_711.py` ruled the 301 on an evidence ladder that uses what
the 2026-08-07 pass never touched: **the 300A carries a street address and ZIP
on 100% of rows and an EIN on 69%**, and none of the three had been read.
**115 filings promoted over 32 tribes** — 54 on street address + ZIP both
matching one Cedar facility (strong enough to carry a `facility_id`), 40 on a
Cedar facility brand, 12 on the filing naming the government, 9 on ZIP + city.
**605 filings over 254 establishments stay `unresolved`**, which ADR-010 makes
an honest record scope and which is the expected majority answer, because NAICS
7132/721120 is the gambling industry and most of it is not tribal. Every one of
the 711 now carries a written ruling, its tribe and its evidence in
`review/employment_osha_711_ruled_2026-09-01.csv`.

**The ordering of that ladder is load-bearing and both orderings were run.**
ZIP-first attributed `HARRAH'S SOUTHERN CALIFORNIA (RINCON)`, 1,400 employees,
to **San Pasqual** (ZIP 92082 in Valley Center CA holds Rincon's Harrah's *and*
San Pasqual's Valley View) and `Twenty Nine Palms Band of Mission Indians`, 720
employees, to **Augustine** (ZIP 92236 holds both Coachella casinos). In each
case the establishment name prints the right owner. The rule that came out of
it — **the record's own words outrank geography, and a bare token may block a
match even though it may never award one** — is written up as rule 7 in
`docs/ENTITY_MATCH_RULES.md`.

**What Cedar still does not hold.** No NLRB election or unfair-labour-practice
records, no QCEW, no BLS wage series, no Wage-and-Hour enforcement, no EEOC, no
OLMS. **NLRB is the one worth building**: tribal casino labour jurisdiction has
been litigated continuously since *San Manuel Indian Bingo & Casino*, 341 NLRB
1055 (2004), the Board's decisions name the tribe and the gaming enterprise
directly in the case caption, and NLRB publishes a public case API
(`api.nlrb.gov`) plus bulk case data with docket, region, case type (`RC`/`CA`),
participant names and dispositions. It would key on the SAME surface this
section already resolves — enterprise names like *Mohegan Tribal Gaming
Authority* and *Casino Pauma* — so the guards written here transfer. Feasibility
assessed, not built.
...
|---|---|---|---|
| **NM per-tribe net win FY2023 – 2026Q2** | 14 quarterly news releases, extracted and **footed against the source's own printed total, 14/14 pass** (`code/216`) | ~~absent~~ **PROMOTED 2026-09-01.** `code/92` now READS the review file; NM went 1,090 → **1,278** rows and reaches **2026-06-30**. All 14 tribes keyed, 0 unresolved. | ✅ done — 188 rows |
| **CA RSTF 93rd report, quarter ending 2024-12-31** | `rstfi__2024__13_RSTF_Distrib_93rd_CommStaffReport-12-31-24.pdf`, **37,974 characters of extractable text** | ~~zero rows~~ **468 rows, 2026-09-01** | ✅ **FIXED.** The defect was a number split across two text spans — `25,438,385.42` rendering as `25,438` + `,385.42` 2.74pt apart on two of 89 rows, which opened a phantom fifth column and made `metric_for` refuse ALL FIVE columns. Rejoined, Exhibit 1 foots to the printed total exactly. |
| **CA RSTF 98th report, quarter ending 2026-03-31** | ~~**0 characters — image-only**~~ **WRONG. Re-measured 2026-09-01: 24,824 characters across 13 pages.** | ~~zero rows~~ **445 rows, 2026-09-01** | ✅ **FIXED, and there was never an OCR job.** From the 98th on, CGCC adds an "Annual Distribution from Revenue Received" column; `metric_for` had no five-column mapping, so every column was refused as `unmapped_column`. **The `0 characters` figure in the previous version of this row was not reproducible and should not be trusted forward.** |
| **CA RSTF 95th / 97th / 99th** (2025-03, 2025-12, 2026-06) | on disk | 112 / 110 / **612** rows | 99th ✅ fixed by the five-column mapping. **95th and 97th remain short and are `CAPTURED_NOT_PARSED`, not a gap**: the 95th's Exhibit 2 inception column is $40,000 off its own printed total (a transposition, 891 vs 851) and the 97th's Exhibit 1 does not foot on four of five columns. Both are recorded with the measured discrepancy in `review/ca_rstf_captured_not_parsed_2026-09-01.md`. **Do not re-fetch them.** |

Measured 2026-09-01, then WORKED the same day by workstream INT-2. The
original diagnosis in this section was right that the corpus was ahead of the
clean table and **wrong about the 98th report**: it is not a scan, it has
24,824 characters, and calling it one would have sent the next session to
build an OCR path it did not need. `REFRESH_CADENCE.md` has been corrected in
place. **Net effect of the repairs: `ca_gaming_payments.csv` 40,164 → 41,758,
and `gaming_capacity_official.csv` 6,461 → 6,649.**

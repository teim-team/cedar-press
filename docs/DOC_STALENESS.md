...
*Generated 2026-09-02 by `code/527_doc_staleness.py`. Two lists, deliberately: a **LIVE** document is one a reader consults for current state, and a wrong number there misleads. A **RECORD** — a build log, a review packet, a dated measurement — was true when written, and its value is that it says what was believed then. Fix the first. Leave the second.*
...
## LIVE documents that are stale — 0

None. Every live document agrees with the data.
...
- `docs/methodology/federal-register.md` L222 — entity count

...
`cedar-consolidated` at `ed730eb`, after #36, #37 and #38 merged:

| | |
|---|---|
| Files to move | **57** — `features/grove` 34, `pages/grove` 21, `components/grove` 1, `styles/grove` 1 |
| Path references to rewrite | **211**, across **59** files |
| Referencing files inside `src/` | 23 |
| Referencing files outside `src/` | 36 — 14 `server/`, 6 `scripts/`, 6 `docs/`, 4 `code/`, 2 `tests/`, and one each of `package.json`, `data/`, `.github/`, `.env.example` |

It is a mechanical rename with no behaviour change, and it should land as its
own commit — moving the four directories to `press/` and rewriting the
references in one pass — for two reasons that are about review rather than
about risk.

First, "did all 211 references get rewritten?" is a question the build, the

...
| | |
|---|---|
| Files to move | 103 — `features/grove` 62, `pages/grove` 38, `components/grove` 1, `styles/grove` 2 |
| Path references to rewrite | 337, across 79 files |
| Referencing files inside `src/` | 38 — `pages/` 30, `features/` 4, `context/` 2, `components/` 1, `main.jsx` 1 |
| Referencing files outside `src/` | 41 — `server/cedar_press/` 8, `scripts/` 7, `docs/` 11, `code/` 5, `server/tests/` 3, `tests/` 2, `data/` 1, `.github/` 1, `package.json` 1, `.env.example` 1, `AGENTS.md` 1 |
...
First, "did all 337 references get rewritten?" is a question the build, the
...
is one of the three things that answer "did all 337 references get rewritten?",

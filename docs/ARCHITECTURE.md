...
| | |
|---|---|
| Files to move | 105 — `features/grove` 64, `pages/grove` 38, `components/grove` 1, `styles/grove` 2 |
| Path references to rewrite | 340, across 81 files |
| Referencing files inside `src/` | 39 — `pages/` 30, `features/` 5, `context/` 2, `components/` 1, `main.jsx` 1 |
| Referencing files outside `src/` | 42 — `server/cedar_press/` 8, `scripts/` 7, `docs/` 11, `code/` 5, `server/tests/` 3, `tests/` 2, `data/` 1, `.github/` 1, `package.json` 1, `.env.example` 1, `AGENTS.md` 1, `eslint.config.js` 1 |
...
First, "did all 340 references get rewritten?" is a question the build, the
...
workspace, and one of those under `docs/` — `DATA_ARCHITECTURE.md` — is
...
is one of the three things that answer "did all 340 references get rewritten?",

...
It should still land as its own commit, moving the four directories to
`press/` and rewriting the references in one pass, for two reasons that are
about review rather than risk.
...
Second, it reaches past the site, and part of what it touches no build would
catch. Four of the referencing files are in `code/`, the Cedar data workspace,
and one of the six under `docs/` — `DATA_ARCHITECTURE.md` — is generated by
`code/500_build_architecture_map.py` and carries a "do not hand-edit" banner:
those references are fixed by editing the script and regenerating, never by
rewriting the output. The rest of what a build cannot see is prose — Markdown,
comments, `.env.example` — which is why `test_rename_plan.py` counts textual
occurrences rather than imports. On the day the four directories move, the
same measurement turns into the stale-path sweep and names every file that
still spells the old one.

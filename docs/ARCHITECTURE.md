...
| | |
|---|---|
| Files to move | 106 — `features/grove` 65, `pages/grove` 38, `components/grove` 1, `styles/grove` 2 |
| Path references to rewrite | 343, across 82 files |
...
First, "did all 343 references get rewritten?" is a question the build, the
...
is one of the three things that answer "did all 343 references get rewritten?",

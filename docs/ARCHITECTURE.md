...
| | |
|---|---|
| Files to move | 118 — `features/grove` 77, `pages/grove` 38, `components/grove` 1, `styles/grove` 2 |
| Path references to rewrite | 368, across 89 files |
| Referencing files inside `src/` | 45 — `pages/` 31, `features/` 10, `context/` 2, `components/` 1, `main.jsx` 1 |
| Referencing files outside `src/` | 44 — `server/cedar_press/` 8, `scripts/` 8, `docs/` 13, `code/` 5, `server/tests/` 3, `tests/` 2, `data/` 1, `.github/` 1, `.env.example` 1, `AGENTS.md` 1, `eslint.config.js` 1 |
...
First, "did all 368 references get rewritten?" is a question the build, the
...
is one of the three things that answer "did all 368 references get rewritten?",

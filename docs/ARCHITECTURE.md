...
| | |
|---|---|
| Files to move | 86 — `features/grove` 52, `pages/grove` 32, `components/grove` 1, `styles/grove` 1 |
| Path references to rewrite | 288, across 70 files |
...
First, "did all 288 references get rewritten?" is a question the build, the
...
is one of the three things that answer "did all 288 references get rewritten?",

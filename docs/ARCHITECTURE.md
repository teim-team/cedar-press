...
**The rename is a named next step, not done here.** Measured on `main` at
`65203ea`, after `cedar-consolidated` landed:

| | |
|---|---|
| Files to move | **57** — `features/grove` 34, `pages/grove` 21, `components/grove` 1, `styles/grove` 1 |
| Path references to rewrite | **213**, across **61** files |
| Referencing files inside `src/` | 23 |
| Referencing files outside `src/` | 38 — 16 `server/`, 6 `scripts/`, 6 `docs/`, 4 `code/`, 2 `tests/`, and one each of `package.json`, `data/`, `.github/`, `.env.example` |

This first read 55 files and 164 references, and deferred the rename because 12
of those files were touched by an open PR. That reason has expired — #36, #37
and #38 are merged — so both the measurement and the justification are retaken
here rather than left standing as a stale reason for a live decision.

It is a mechanical rename with no behaviour change, and it should still land as
its own commit — moving the four directories to `press/` and rewriting the
references in one pass — for two reasons that are about review rather than
about risk.

First, "did all 213 references get rewritten?" is a question the build, the
suites and the smoke run answer, and not one a reader can answer by reading a
diff. Folded into a change that also alters behaviour or prose, it hides that
change instead of accompanying it.

Second, the rename reaches past the site. Four of the referencing files are in
`code/`, the Cedar data workspace, and one of the six under `docs/` —
`DATA_ARCHITECTURE.md` — is generated by `code/500_build_architecture_map.py`
and carries a "do not hand-edit" banner. Those references are fixed by editing
the script and regenerating, never by rewriting the output, so the rename has a
verification step of its own that has nothing to do with the web client.

...
*(and `174 facilities *(**CORRECTED 2026-09-02:** 174 is the count of the ASSERTION `SEALED_BY_STATUTE_OR_COMPACT`, not of the evidence. On the same rows, **113** carry `SEALED_HELD_BY_REGULATOR`; **58** carry `DISPOSITION_UNSUPPORTED_BY_RECORDED_QUOTE` (MN 48, NV 10) and **3** carry `NOT_COLLECTED_BY_THIS_BODY` (CO) — a body that does not hold the figure has sealed nothing.)*` in the row below is the count of the ASSERTION `SEALED_BY_STATUTE_OR_COMPACT`, not of the evidence: 113 are `SEALED_HELD_BY_REGULATOR`, 58 `DISPOSITION_UNSUPPORTED_BY_RECORDED_QUOTE` (MN 48, NV 10), 3 `NOT_COLLECTED_BY_THIS_BODY` (CO).)*

| `gaming_facilities.csv` | `23d_build_gaming_facilities.py` | `960_promote_gaming_facility_class_and_revenue_reach.py` | not yet paid - declared at creation. Any rebuild of the facility table drops the eleven columns 960 appends, and with them the only path from a facility to `gaming_revenue_bounds.csv` (694 of 787 rows), the only statement of Class II / Class III on the record (684 of 787), and the quoted statute or compact clause that explains why 174 facilities in seven states can never carry a revenue figure. Re-run 960 after any facility rebuild; the `.bak_<date>_pre960` beside the table is the signal |

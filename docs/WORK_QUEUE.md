...
| 6 | ~~`NOT_ACQUIRED`~~ → **`ON_DISK_NOT_PROMOTED`, and DONE the same evening** | **This row was wrong within the hour, and the way it was wrong is the point.** It said bill ACTIONS needed ~3,000 congress.gov requests. `data/clean/_bill_actions.csv` already held **31,936 actions over 3,061 bills**, fetched 2026-08-06, `ok` on all 3,061 — a second leading-underscore orphan in the same directory as `_cosponsors.csv`, invisible for the same reason: the name matches no `COLLECTIONS` pattern. Promoted by `code/1150_bill_actions_promote.py` with **zero network** into `native_bill_actions.csv` + `native_bill_action_coverage.csv`. **283 of 3,069 Native bills became law.** Still genuinely `NOT_ACQUIRED`: bill COMMITTEES and SUBJECTS | `docs/COSPONSOR_HARVEST_LOG_2026-09-02.md` |

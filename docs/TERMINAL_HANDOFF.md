...
| 3 | **Re-run linkage coverage whenever a flagship changes.** `py -3 code/1139_linkage_coverage.py apply` | `docs/LINKAGE_COVERAGE.md` | `npm test` passes in `cedar-press`. It reads that file and fails if the headline total or either named extreme has moved. |

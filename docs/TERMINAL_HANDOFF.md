...
| 11 | **Decide when `Native Federal Advocacy & Engagement` loses its ampersand.** The Lumecon copy rule is that a reader never sees `&`; write "and". This name breaks it, and it cannot simply be edited, because `collectionCitation` (`src/features/grove/collection.js`) embeds `dataset.name` verbatim into the citation string that is **written as the last row of every downloaded CSV**. Subscribers already hold files whose citation carries the current name, so renaming it now makes two downloads of the same dataset at the same version cite it differently. Three things move together when it does change: `name` in `src/features/grove/pressCatalog.js`, a regenerated `server/cedar_press/_press_data.json` (`node scripts/dump-press.mjs > server/cedar_press/_press_data.json`, which `server/tests/test_collection.py` fails the build over if stale), and the Python mirror of the citation function, which that file's docstring says is compared byte for byte. The cheapest moment is a **version bump on this collection**, where the citation string legitimately changes anyway. Checked 2026-09-18: this is the only collection name carrying an ampersand. | `src/features/grove/pressCatalog.js` (`id: "lobbying"`) | Either the name is changed at a version bump with the JSON and the Python mirror regenerated in the same change, or the owner records that published names are exempt from the copy rule. |

**Nothing else.** No dataset, schema, or publication rule changed on 2026-09-14;
every open decision is items 1, 2, 7, 9, 10 and 11.

...
*Written 2026-08-28 by `code/391_triage_unshipped_tables.py`. Re-runnable; it reads `data/clean` live rather than a snapshot.*
...
## NEEDS_A_RULING — 3 table(s), 2,441 rows
...
| `cedar_correction_register.csv` | 163 | — | written IN PLACE right now by the lobbying-correction pass (scripts 350-358), which AGENTS.md names as the live owner of the failing registry metrics in 62. QUESTION: does the correction register publish as a transparency artefact, and does its owner want it registered? Not mine to answer while they are mid-pass |
...
## SHIP — 84 table(s), 489,687 rows

| table | rows | block | why |
|---|---:|---|---|
| `fr_content_classification.csv` | 156,452 | `09b_fr_content_classification` | docs/CONTENT_ANALYSIS.md 'Outputs' lists this under Series, which that document separates by name from its 'Audit and accuracy' files |
...
| `fpds_uei_cage_map.csv` | 24,977 | `02n_fpds_uei_cage_map` | already curated into 25_build_publication_layer.TABLES and shipping into cedar_press.db; the only thing missing is a notes contract |
...
| `cross_dataset_ruling_map.csv` | 7,507 | `05f_cross_dataset_ruling_map` | already curated into 25_build_publication_layer.TABLES and shipping into cedar_press.db; the only thing missing is a notes contract |
...
| `fr_nagpra_title_index.csv` | 6,606 | `11b_fr_nagpra_title_index` | docs/CONTENT_ANALYSIS.md 'Outputs' lists this under Series, which that document separates by name from its 'Audit and accuracy' files |
...
| `fpds_uei_edges.csv` | 2,290 | `02o_fpds_uei_edges` | already curated into 25_build_publication_layer.TABLES and shipping into cedar_press.db; the only thing missing is a notes contract |
...
| `native_passthrough.csv` | 1,262 | `02q_native_passthrough` | Native prime to Native sub passthrough, award grain |
...
| `fr_consultation_notices.csv` | 484 | `09j_fr_consultation_notices` | already ships, but under 09_federal_actions at 0.636. Given its own block so it is neither mis-documented nor outranked by 09b |
...
| `native_passthrough_pairs.csv` | 212 | `02r_native_passthrough_pairs` | the same passthrough, entity-pair grain |
...
| `admin_appeal_positions.csv` | 1 | `04y_admin_appeal_positions` | positions taken before the boards |
...
## INTERNAL — 56 table(s), 109,097 rows
...
| `entity_evidence_profile.csv` | 1,282 | — | a self-measurement of Cedar's own collection - what we swept, what answered, how much we covered. A fact about us, not about Indian Country; its own column amounts_per_source_NEVER_SUM says what it is for |
...
| `assistance_tribe_id_crosswalk.csv` | 361 | — | a PROPOSAL that is deliberately not applied. START_HERE.md records that 152 and 24 both decline to write it in - 'the NEID crosswalk is a ruling, not a computation' - and 122 of its 344 candidates come from the containment matcher AGENTS.md forbids from keying a dollar |
...
| `gaming_field_coverage.csv` | 110 | — | a self-measurement of Cedar's own collection - what we swept, what answered, how much we covered. A fact about us, not about Indian Country |

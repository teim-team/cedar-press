...
| EMPTY | 4 | 0 |
> **CORRECTED 2026-09-02 — `nest` is READY and shipped.**
> `docs/DATASET_READINESS.md` (regenerated by `code/518_dataset_readiness.py`)
> records `nest` as **READY**: 2 tables, 2/2 contracts, 2/2 plans, lint clean.
> `nest_enterprises.csv` holds **1,610 enterprises**, and the headline finding
> is that **977 of them (60.7%) are absent from federal contracting** — which is
> the product, not a gap: those 977 are enterprises no federal dataset can see.
> `nest__sample.csv` (10 rows, 17 columns) and the full editorial copy in
> `docs/datasets/_descriptors.json` both shipped on the previous push, so any
> line here saying `nest` still needs a sample and copy is stale.
>
> **`READY 2 / 13` is also stale** — `START_HERE.md` carries a note saying so
> and naming the scoreboard as the authority. Do not quote a readiness count
> from prose; run `py -3 code/518_dataset_readiness.py`.

...
| EMPTY | 4 | 0 |
> **CORRECTED 2026-09-02 — `need` is READY and shipped.**
> `docs/DATASET_READINESS.md` (regenerated by `code/518_dataset_readiness.py`)
> records `need` as **READY**: 2 tables, 2/2 contracts, 2/2 plans, lint clean.
> `need_enterprises.csv` holds **1,610 enterprises**, and the headline finding
...
> `need__sample.csv` (10 rows, 17 columns) and the full editorial copy in
> `docs/datasets/_descriptors.json` both shipped on the previous push, so any
> line here saying `need` still needs a sample and copy is stale.
>
